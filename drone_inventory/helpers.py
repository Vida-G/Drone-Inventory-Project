...
#         try:
#             current_user_token = User.query.filter_by(token = token)
#             print(token)
...
#             if token != owner.token and secrets.compare_digest(token, owner.token):
#                 return jsonify({'message': 'Invalid token try again'})
#         return our_flask_function(current_user_token, *arg, **kwrgs)
#     return decorated
...
# class JSONEncoder(json.JSONEncoder):
#     def default(self, obj):
#         # convert anu decimal values into string to allow data to be passed through API 
#         if isinstance(obj, decimal.Decimal):
#             #convert that decimal into string
#             return str(obj)
#         return super(JSONEncoder, self).default(obj)
